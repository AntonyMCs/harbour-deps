...
[ -f "pkg/zlib${_patsuf}.patch" ] && dos2unix < "pkg/zlib${_patsuf}.patch" | patch -N -p1 -d zlib
...
if [ "${_BRANCH#*cares*}" != "${_BRANCH}" ]; then
  ...
  [ -f "pkg/c-ares${_patsuf}.patch" ] && dos2unix < "pkg/c-ares${_patsuf}.patch" | patch -N -p1 -d c-ares
fi
...
if [ "${_BRANCH#*libressl*}" != "${_BRANCH}" ]; then
  ...
  [ -f "pkg/openssl${_patsuf}.patch" ] && dos2unix < "pkg/openssl${_patsuf}.patch" | patch -N -p1 -d openssl
fi
...
[ -f "pkg/libssh2${_patsuf}.patch" ] && dos2unix < "pkg/libssh2${_patsuf}.patch" | patch -N -p1 -d libssh2
...
[ -f "pkg/curl${_patsuf}.patch" ] && dos2unix < "pkg/curl${_patsuf}.patch" | patch -N -p1 -d curl
